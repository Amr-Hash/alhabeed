#!/usr/bin/env node
/**
 * Generate VAPID keys and set them on the alhabeed-api Vercel project (production).
 * Usage: node scripts/setup-vapid-vercel.mjs
 * Optional: VERCEL_TOKEN, VAPID_ADMIN_EMAIL
 */
...
const ADMIN_EMAIL = process.env.VAPID_ADMIN_EMAIL?.trim() || "noreply@alhabeed.com";
...
const keys = JSON.parse(gen.stdout.trim());
...
console.log("Done. Verify: https://alhabeed-api.vercel.app/api/notifications/push/vapid-public-key");
